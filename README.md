...
Imports `load_multiclass_mask` from `util.py`, which rasterises the polygon shapes from a LabelMe `.json` into an integer mask array (0 = background, 1 = defect) the same size as the original image. This is how `.json` annotations become `.png` masks the network can train on. The function source lives in `util.py` so the notebook stays focused on the *what* rather than the *how* — open `util.py` if you want to read or modify the rasterisation logic.

```python
# ============================================================
# Helpers: mask from LabelMe JSON (defined in util.py)
# ============================================================
from util import load_multiclass_mask

print('Helpers imported from util.py.')
```

### Step D. Augmentation helpers

Imports the augmentation tag list (`AUG_TAGS`) and the `augment_array` helper from `util.py`. `augment_array` applies the same flip/rotation to image and mask in lock-step. **Full augmentation uses all 6 transforms** — `orig`, `fliplr`, `flipud`, `rot90`, `rot180`, `rot270` — which multiplies the training set 6× and is one of the easiest places to lift defect IoU (see Q3). For a faster baseline run, edit `AUG_TAGS` in `util.py` to drop back to `['orig', 'fliplr']`, or comment/uncomment the per-tag branches inside `augment_array`.

```python
# ============================================================
# Augmentation helpers (defined in util.py)
# ============================================================
from util import AUG_TAGS, augment_array

print(f'Augmentation helpers imported from util.py. AUG_TAGS = {AUG_TAGS}')
```
...
```python
# ============================================================
# Collect image/json pairs (helper defined in util.py)
# ============================================================
from util import collect_pairs
...
The U-Net architecture lives in `util.py` and is imported here. It consists of a `DoubleConv` block (Conv → GroupNorm → ReLU, twice, with optional dropout), a 4-level encoder/decoder with channel widths `32 → 64 → 128 → 256 → 512`, and **skip connections** that concatenate matching encoder features into each decoder block. The final `1×1` conv emits per-class logits at full resolution. See Q5 for why skip connections are crucial for pixel-wise tasks.

Open `util.py` and read the `DoubleConv` / `UNet` classes once — that is the architecture you are training. Keeping the model code in one place also means Part 3 inference loads the *exact same* class definition (no risk of the two copies drifting out of sync).

```python
from util import UNet

# Quick sanity print: how many parameters does this U-Net have?
_model = UNet(num_classes=2)
n_params = sum(p.numel() for p in _model.parameters())
print(f'UNet imported from util.py — {n_params:,} parameters')
del _model
```
...
from util import decode_predictions  # shared by training, validation, and Part 3 inference
...
# decode_predictions is imported from util.py; pass CLASS_CONFIDENCE_THRESHOLDS explicitly
# so the same helper works at training, validation, and inference time.
def accumulate_iou(intersection, union, logits, targets):
    preds = decode_predictions(logits, CLASS_CONFIDENCE_THRESHOLDS)
...
# PALETTE and mask_to_rgb live in util.py (shared with Part 3 inference)
from util import PALETTE, mask_to_rgb
...
### Step B. Re-import U-Net (standalone for inference)

Re-imports the `UNet` class from `util.py` so that **Part 3 can be run standalone** (e.g. on a fresh kernel) without re-running the training cells. Because both Part 2 and Part 3 share the same definition in `util.py`, the architecture is guaranteed to match the one used to save `unet_best.pt` — no risk of two copies drifting out of sync.
...
All four helpers — `decode_predictions`, `pad_to_multiple`, `mask_to_rgb`, `predict_image`, plus the `find_image_path` utility and the `PALETTE` constant — are imported from `util.py` (the same module Part 2 used). Quick reminders:

- `decode_predictions` applies the per-class confidence threshold (predictions below threshold collapse to background).
- `pad_to_multiple` reflect-pads the input to a multiple of 16 (since the U-Net halves resolution 4 times) and remembers the original size so the mask can be cropped back.
- `predict_image` runs the model on a full image (no patching at test time) and returns the RGB display image, the integer mask, and the defect-probability heat-map.
...
for image_id in TEST_IMAGE_IDS:
    image_path = find_image_path(image_id, SOURCE_DIR)
    image, pred_mask, defect_prob = predict_image(image_path, model, device)
